...
class PromptEnhancerApp:
    ...
    def update_output(self, text, success):
        ...
        self.root.after(10, self._adjust_window_to_fit_output)
    ...
    def _adjust_window_to_fit_output(self):
        """Adjusts the window height to dynamically fit the output text content."""
        ...
        visual_lines = 0
        widget_width = self.output_text.winfo_width()
        if widget_width <= 1:
            widget_width = self.fixed_width - 40

        font_obj = font.Font(font=self.output_text['font'])
        char_width = font_obj.measure('0')
        chars_per_line = max(1, (widget_width - 10) // char_width)

        for line in content.split('\n'):
            visual_lines += (len(line) + chars_per_line - 1) // chars_per_line if line else 1
        
        if content.endswith('\n'):
            visual_lines -= 1

        new_height_lines = max(3, min(visual_lines, 30))
        self.output_text.config(height=new_height_lines)
        self.root.update_idletasks()
    ...
    def clear_text(self):
        ...
        if self.output_label.winfo_ismapped():
            ...
            # Window will shrink automatically as widgets are removed
            self.root.update_idletasks()
